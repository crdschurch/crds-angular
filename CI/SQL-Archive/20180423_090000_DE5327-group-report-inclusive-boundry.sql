...
CREATE OR ALTER PROCEDURE [dbo].[report_CRDS_Group_Participants_By_Date_Congregation] 
     @reportdate DATE
AS
...
select G.Group_ID, count(GP.Participant_ID) As Participant_Count , C.Congregation_Name from Group_Participants GP 
    ...
    where
	((@reportdate >= CONVERT(date,GP.Start_Date)) AND
	(GP.End_Date is null OR @reportdate <= CONVERT(date,GP.End_Date)))
	AND
	((@reportdate >= CONVERT(date,G.Start_Date)) AND
	(G.End_Date is null OR @reportdate <= CONVERT(date,G.End_Date)))
	AND
