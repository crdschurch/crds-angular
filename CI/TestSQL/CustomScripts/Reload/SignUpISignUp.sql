...
SET @groupID = (SELECT Group_ID FROM [dbo].Groups WHERE Group_Name = '(t+auto) Verify UI')
SET @participantID = (SELECT Participant_ID FROM [dbo].Participants 
...
BEGIN
	...
	INSERT INTO [dbo].Responses (Response_Date, Opportunity_ID, Participant_ID, Response_Result_ID, Domain_ID, Event_ID, Closed)
	VALUES (GETDATE(), @opportunityID, @participantID, 1, 1, @ID, 0)

	INSERT INTO [dbo].Event_Participants (Event_ID, Participant_ID, Participation_Status_ID, Group_ID, Domain_ID)
