...
IF NOT EXISTS ( SELECT  *
	...
	@online_registration_product_name nvarchar(50),
...
BEGIN
	...
	IF @online_registration_product_name is not null
	BEGIN
		DECLARE @product_id INT = (SELECT TOP 1 Product_ID FROM [dbo].Products WHERE Product_Name = @online_registration_product_name ORDER BY Product_ID ASC);
		IF @product_id is null
		BEGIN
			SET @error_message = 'Could not find product with name '+@product_id+ '. Online Registration Product will not be added to event.'+CHAR(13);
		END;
	END
	...
	BEGIN
		...
		(Event_Title,Event_Type_ID ,Congregation_ID ,Program_ID ,Primary_Contact,Event_Start_Date,Event_End_Date,Minutes_for_Setup,Minutes_for_Cleanup,Cancelled ,Visibility_Level_ID,Featured_On_Calendar ,[Allow_Check-in],Ignore_Program_Groups ,Prohibit_Guests ,Send_Reminder ,Domain_ID,On_Donation_Batch_Tool,Force_Login ,Registration_Start      ,Registration_End      ,Online_Registration_Product) VALUES
		(@event_name,@event_type_id,@congregation_id,@program_id,@contact_id    ,@start_date     ,@end_date     ,@setup_time      ,@cleanup_time      ,@cancelled,@visibility_level  ,@featured_on_calendar,@allow_checkin  ,@ignore_program_groups,@prohibit_guests,@send_reminder,1        ,@on_batch_tool        ,@force_login,@registration_start_date,@registration_end_date,@product_id                );
	...
	BEGIN
		...
		Online_Registration_Product = @product_id
