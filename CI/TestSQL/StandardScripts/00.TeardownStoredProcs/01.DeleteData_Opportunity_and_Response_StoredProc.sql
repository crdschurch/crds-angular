...
IF NOT EXISTS ( SELECT  *
	...
	@opportunity_id int AS SET NOCOUNT ON;')
GO
...
-- Defines cr_QA_Delete_Opportunity_By_Name
IF NOT EXISTS ( SELECT  *
	FROM    sys.objects
	WHERE   object_id = OBJECT_ID(N'cr_QA_Delete_Opportunity_By_Name')
			AND type IN ( N'P', N'PC' ) )
	EXEC('CREATE PROCEDURE dbo.cr_QA_Delete_Opportunity_By_Name
	@opportunity_name nvarchar(50) AS SET NOCOUNT ON;')
GO
ALTER PROCEDURE [dbo].[cr_QA_Delete_Opportunity_By_Name]
...
BEGIN
	...
	--Delete all opportunities with name
	DECLARE @opportunities_to_delete TABLE
	(
		opportunity_id int
	)
	INSERT INTO @opportunities_to_delete (opportunity_id) SELECT Opportunity_ID FROM [dbo].Opportunities 
	WHERE Opportunity_Title = @opportunity_name;
	...
	BEGIN
		--Get top item in list
		Set @cur_entry_id = (SELECT TOP 1 opportunity_id 
			FROM @opportunities_to_delete
			WHERE opportunity_id > @cur_entry_id
