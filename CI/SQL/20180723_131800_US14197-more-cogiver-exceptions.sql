...
AS
    -- This CTE helps find Households with multiple Heads of Household
    WITH Multi_HoH_Households_CTE (Household_ID, Row_Num)
    AS
    (
        SELECT
            Household_ID,
            Row_Num = ROW_NUMBER() OVER (PARTITION BY Household_ID ORDER BY Contact_ID)
        FROM
            Contacts c
        WHERE
            c.Household_Position_ID = 1     -- head of household
    ...
    from
        contacts c1
        inner join contacts c2 on c2.household_id = c1.household_id and c1.contact_id < c2.contact_id
        left join Contact_Relationships cr on cr.Contact_ID = c1.Contact_ID and cr.Relationship_ID = 42 and cr.End_Date is null
    where
        c1.Household_Position_ID = 1
        and c2.Household_Position_ID = 1
        and c1.Contact_Status_ID = 1    -- Active
        and c2.Contact_Status_ID = 1    -- Active
        and cr.Contact_Relationship_ID is null
        -- The check for Household_ID below is only for query performance.  Without this additional condition,
        -- the query will return the same results, but will take 7-8 seconds vs. 500ms with this condition.
        and c1.Household_ID IN (SELECT Household_ID FROM Multi_HoH_Households_CTE WHERE Row_Num = 2)
    ;
GO
