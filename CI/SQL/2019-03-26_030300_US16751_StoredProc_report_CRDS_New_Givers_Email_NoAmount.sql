...
/****** Object:  StoredProcedure [dbo].[report_CRDS_New_Givers_NoAmount]    Script Date: 3/7/2019 12:53:48 PM ******/
SET ANSI_NULLS ON
GO

SET QUOTED_IDENTIFIER ON
GO
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'report_CRDS_New_Givers_NoAmount')
DROP PROCEDURE [dbo].[report_CRDS_New_Givers_NoAmount]
GO
...
EXEC	@return_value = [dbo].[report_CRDS_New_Givers_NoAmount]
		@startdate = N'2019-01-01',
		@enddate = N'2019-01-15',
		@programid = N'3',
		@congregationid = N'1',
		@donationstatusid = N'2',
		@accountingcompanyid = N'1'
...
CREATE   PROCEDURE [dbo].[report_CRDS_New_Givers_NoAmount] 
     @startdate DATETIME,
     @enddate DATETIME,
...
insert into #ftdonordetail(hhid,donation_id,donation_date,firstdononationdate,donation_amount,programid, programname,cid1,cbday,cdisplayname,cfname,clname,cemail,cid2,cgbday,cgdisplayname,cgfname,cglname,cgemail, addr1,addr2,city,st,zip,site,statementtype,donationstatus,sddonid)
...
c.Contact_ID,c.Date_of_Birth,c.Display_Name,c.First_Name,c.Last_Name,c.Email_Address,c2.Contact_ID,c2.Date_of_Birth,c2.Display_Name as cogivername,c2.First_Name,c2.Last_Name,c2.Email_Address,
...
update #ftdonordetail set cemail ='' where cemail  is null
...
update #ftdonordetail set cgemail ='' where cgemail  is null
