...
IF EXISTS(SELECT name FROM sys.objects WHERE name = N'api_crds_Get_Finder_AWS_Data_For_Single_Group')
  DROP PROCEDURE dbo.api_crds_Get_Finder_AWS_Data_For_Single_Group;
GO
...
WHERE ((G.Group_Type_ID IN (@anywhereGroupTypeId) AND G.Available_Online = 1 AND P.Host_Status_ID = @approvedStatusId AND (G.End_Date IS NULL OR G.END_DATE > GETDATE())) OR --ANYWHERE GATHERINGS
      (G.Group_Type_ID IN (@smallGroupTypeId) AND G.Available_Online = 1 AND G.Group_Is_Full = 0 AND (G.End_Date IS NULL OR G.END_DATE > GETDATE()))) AND G.Group_ID = @GroupId                            --SMALL GROUPS
