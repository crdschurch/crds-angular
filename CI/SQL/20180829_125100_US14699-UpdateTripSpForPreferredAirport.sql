...
GO
/****** Object:  StoredProcedure [dbo].[report_CRDS_Trip_Export]    Script Date: 8/28/2018 2:15:19 PM ******/
SET ANSI_NULLS ON
GO
...
DECLARE @preferredairport INT = 9050;
...
DECLARE @backgroundcheck INT = 20;
  ...
  (SELECT STUFF(
   ...
   (SELECT TOP 1 notes
	...
	(SELECT top 1  response
		...
		AND r.form_response_id = fr.form_response_id
		ORDER BY r.Form_Response_Answer_ID desc) AS medicalconditions,
...
(SELECT top 1 (CASE d.received WHEN 0 THEN 'No' WHEN 1 THEN 'Yes' END) AS docreceived
   FROM dbo.cr_eventparticipant_documents d
   WHERE d.event_participant_id = ep.event_participant_id
     AND d.document_id = @backgroundcheck) AS backgroundcheck,
